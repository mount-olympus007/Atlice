...
namespace Atlice.WebUI.Models
{
    public class WebSocketMiddleware
    {
        ...
        private async Task HandleMessage(WebSocket socket, string message)
        {
            ...
            {
                if(clientMessage.Content == "Ping")
                {
                    string expectedClient = _webSocketHandler.ValidateUsername(clientMessage.Receiver);
                    if (string.IsNullOrEmpty(expectedClient))
                    {
                        ServerMessage chatMessage = new ServerMessage(clientMessage);
                        chatMessage.Content = "User not Connected.";
                        await _webSocketHandler.BroadcastMessage(JsonSerializer.Serialize(chatMessage));
                    }
                    else
                    {
                        clientMessage.Content = "Is online.";
                        ServerMessage chatMessage = new ServerMessage(clientMessage);
                        await _webSocketHandler.BroadcastMessage(JsonSerializer.Serialize(chatMessage));
                    }
                }
            ...
            else if (clientMessage.IsTypeChat())
            {
                ...
                if(clientMessage.Sender == "Admin")
                {
                    if(expectedUsername == "lexdamas@gmail.com" || expectedUsername == "allen.c.bradford@gmail.com")
                    {
                        ServerMessage chatMessage = new ServerMessage(clientMessage);
                        await _webSocketHandler.BroadcastMessage(JsonSerializer.Serialize(chatMessage));
                    }
