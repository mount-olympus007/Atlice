(function($) {
    ...
    blog.submitComment = submitComment;
    blog.openReplyForm = openReplyForm;
    ...
    */
    ...
    function eltdInitAjaxPagination(container){
        ...
        if(nextPage <= maxNumPages){
            ...
            $.ajax({
                type: 'GET',
                data: { NextPage: ajaxData.nextPage, Number: ajaxData.number, catName: ajaxData.catId, TagSlug: ajaxData.tagSlug, SearchValue: ajaxData.searchValue },
                url: "/Digest/Index",
                success: function (data) {
                    nextPage++;
                    container.data('next-page', nextPage);
                    var responseHtml = data.listHTML.result;
    ...
    */
        ...
        if ($('.eltd-expandable-tiles').length) {
            ...
            eltdET.show_outer_page = function ($link) {
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        eltdET.new_url = document.location.origin + "/";
                    ...
                    if (next.is('.expanded') || $(this).attr('href').split('#')[0] !== post_url) {
                        ...
                        if (eltdET.busy) {
                            ...
                            $.ajax({
                                type: 'POST',
                                async: true,
                                data: { ArticleID: currentItemId },
                                url: "Home/Article",
                                success: function (data) {
                                    var mainURL = "https://mastergrind.org/?ArticleID=" + currentItemId;
                                    

                                    var responseHtml = data.viewHTML;
            ...
            eltdET.update_grid = function(params) {
                ...
                if (typeof params !== 'object') {return; }
                    ...
                    //var blogHolder = $('.eltd-blog-holder');
                    //blogHolder.data('cat-id', params.catId);
                    //blogHolder.data('tag-slug', params.tagSlug);
                    //blogHolder.data('next-page', params.nextPage);
                }
    ...
    function eltdGetContactForm() {
        var selector = $('#myContactFormLink');
        selector.off('click').on('click', function (e) {
            e.preventDefault();
            e.stopPropagation();
    ...
    */
    function eltdGetCategoryHtml(catID){
        ...
        $.ajax({
            type: 'POST',
            data: { catName: ajaxData.currentCatId, NextPage: 1 },
            url: "Home/Index",
            success: function (dataReturned) {
                var response = dataReturned;
                var responseHtml = response.listHTML;
    ...
    function openReplyForm() {
        $(".comment-reply-link").click(function (e) {
            ...
            input.setAttribute("name", "comment_parent");
            input.setAttribute("type", "hidden");
            input.setAttribute("value", parentCommentID);
            var replyForm = $(".commentform").first().clone();
            replyForm.append(input);
            replyForm.appendTo(this.parentElement);



            replyForm.submit(function (e) {
                var member = $('input[name=comment_post_ID]').attr("value");
                ...
                var commentP = $('input[name=comment_parent]').attr("value");
                var commentDiv = $("#comments"); //form.parent().parent().siblings().find(".eltd-comment-holder clearfix");
                $.ajax({
                    type: "POST",
                    url: ElatedAjaxUrl,
                    data: { comment_parent: commentP, comment_post_ID: member, url: url, email: email, author: author, comment: comment }, // serializes the form's elements.
                    success: function (data) {
                        alert(data);
    ...
    */
    function eltdGetTagHtml(tagSlug){
        ...
        $.ajax({
            type: 'POST',
            data: { TagSlugID: ajaxData.currentTagSlug, NextPage: 1 },
            url: "Home/Index",
            success: function (data) {
                
                var responseHtml = data.listHTML;

                //reinit load more params                        
                blogHolder.data('max-pages', data.maxpages);
                eltd.modules.blog.eltdExpandableTiles.update_grid({
                    action: 'refresh',
                    html: responseHtml
